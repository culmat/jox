...
/**
 * The PnutsLayout is a general purpose geometry manager. It is very easy to use
 * and can accomplish any grid based layout.
 * 
 * <pre>
 ...
 * <tr>
 * <td>uniform</td>
 * <td>Sets the width or height to be the same for each column. Can be
 * x/y/xy/none.</td>
 * <td>none</td>
 * <td>true</td>
 * <td>false</td>
 ...
 * <tr>
 * <td>spacing</td>
 * <td>Minimum spacing that will be put around the component. Can be a single
 * value (5) or top:right:bottom:left (eg 5:10:5:10).</td>
 * <td>0</td>
 * <td>true</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>padding</td>
 * <td>Padding around the component. Can be a single value (5) or
 * top:right:bottom:left (eg 5:10:5:10).</td>
 * <td>0</td>
 * <td>true</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>ipadding</td>
 * <td>Padding inside the component (making it larger). Can be a single value
 * (5) or x:y (eg 5:10).</td>
 * <td>0</td>
 * <td>true</td>
 * <td>true</td>
 ...
 * <tr>
 * <td>fill</td>
 * <td>Sets the width and/or height of the component to a percentage of the cell
 * width. Can be x/y/xy/none or a single value (5) or x:y (eg 25:75). 0 or none
 * sizes the component to its preferred size.</td>
 * <td>none</td>
 * <td>true</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>align</td>
 * <td>Alignment of the component in the cell. Can be
 * top/bottom/left/right/center or top/bottom/center:left/right/center (eg
 * top:right).</td>
 * <td>center</td>
 * <td>true</td>
 * <td>true</td>
 ...
 * <tr>
 * <td>expand</td>
 * <td>Expands the size of the cell so the table takes up the size of the
 * container. Can be x/y/xy/none.</td>
 * <td>none</td>
 * <td>true</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>border</td>
 * <td>Draws the grid borders of the table. Great for debugging. Can be
 * true/false.</td>
 * <td>false</td>
 * <td>true</td>
 * <td>true</td>
...
public class PnutsLayout implements LayoutManager2, Serializable {
	...
	/*
	 * Constructs a PnutsLayout from a constraint String. See PnutsLayout class
	 * comments for constraints usage.
	 */
	...
	/**
	 * Constructs a PnutsLayout from a constraint Map. See PnutsLayout class
	 * comments for constraints usage.
	 * 
	 * @param map
	 */
	...
	/**
	 * Sets the constraints for a component. See PnutsLayout class comments for
	 * constraints usage.
	 */
	...
	private void bindContainer(Container target) {
		...
		for (int i = 0, gridX = 0, gridY = 0; i < compCount; i++) {
			...
			if (comp instanceof BorderPanel)
				continue;
		...
		for (int i = 0; i < compCount; i++) {
			...
			if (colspan == 1) {
				if (compWidth > widths[gridX])
					widths[gridX] = compWidth;
				if (maxWidth < widths[gridX])
					maxWidth = widths[gridX];
			} else {
				for (int ii = 0; ii < colspan; ii++) {
					if ((xExpand[gridX + ii] & 1) != 0) { // expand is set
						int expandedWidth = (compWidth - no_expand_widths) / expand_count;
						if (expandedWidth > widths[gridX + ii])
							widths[gridX + ii] = expandedWidth;
					}
					if (xFixed && maxWidth < widths[gridX + ii])
						maxWidth = widths[gridX + ii];
				}
			}

			if (heights[gridY] < (compHeight + padding[0] + padding[2]) / rowspan) {
				heights[gridY] = (compHeight + padding[0] + padding[2]) / rowspan;
				if (yFixed && maxHeight < heights[gridY])
					maxHeight = heights[gridY];
			}
		}
	...
	/**
	 * Lays out the container. This method will actually reshape the components in
	 * the target in order to satisfy the constraints of the PnutsLayout object.
	 * 
	 * @see Container
	 */
	public void layoutContainer(Container target) {
		...
		int ah = 0;
		if (xExpandCount > 0)
			aw = (targetWidth - preferredSize.width) / xExpandCount;
		if (yExpandCount > 0)
			ah = (targetHeight - preferredSize.height) / yExpandCount;
		...
		for (int i = 0; i < compCount; i++) {
			...
			if (fill[0] > 0)
				compWidth = (int) ((columnLeft[gridX + colspan] - columnLeft[gridX] - (padding[3] + padding[1]))
						* (fill[0] / 100F));
			if (fill[1] > 0)
				compHeight = (int) ((rowTop[gridY + rowspan] - rowTop[gridY] - (padding[0] + padding[2]))
						* (fill[1] / 100F));
	...
	/**
	 * Returns the constraints used by the table and the default constraints used
	 * for all cells in the table. Modifying this object will modify the table's
	 * constraints.
	 */
	 ...
	 * Modifying this object will modify the cell's constraints.
	 */
	...
	public Rectangle getGridRectangle(Container parent, int index) {
		...
		int colspan = constraints.getColspan();
		int rowspan = constraints.getRowspan();
		return new Rectangle(cellWidth, cellHeight, columnLeft[gridX + colspan] - cellWidth,
				rowTop[gridY + rowspan] - cellHeight);
	}
	 ...
	 * 
	 * @param obj
	 *            Map or String defining constraints for the component.
	 */
	...
	/**
	 * Invalidates the layout, indicating that if the PnutsLayout has cached
	 * information it should be discarded. This happens automatically when cell or
	 * table constraints are modified.
	 */
	...
	static Map str2table(String constraintString) {
		...
		while (constraints.hasMoreTokens()) {
			...
			if (constraintString.indexOf('=') == -1)
				throw new IllegalArgumentException("Invalid constraint name/value pair: " + token);
	 ...
	 * @param obj
	 *            Must be a String.
	 * @return An Integer representing the passed in String, or null if the String
	 *         is null.
	 * @throws NumberFormatException
	 *             if the String could not be parsed into an Integer.
	 */
	...
	public abstract class Constraints {
		...
		/**
		 * Configures this contraint object with a Map. See PnutsLayout class comments
		 * for constraints usage.
		 */
		public void configure(Map map) {
			...
			if (border != null)
				setBorder(new Boolean("true".equals(border)));
			...
			if (map.get("align") == null) {
				...
				if (alignString != null)
					setAlign(alignString);
			}
		}

		public int getAlign() {
			if (align == null)
				return tableConstraints.getAlign();
			return align.intValue();
		}

		/**
		 * Can be TOP, RIGHT, BOTTOM, LEFT, or any combination of TOP/BOTTOM and
		 * RIGHT/LEFT.
		 */
		public void setAlign(Integer align) {
			if (align == null && this == tableConstraints)
				return;
			this.align = align;
			invalidate();
		}
		...
		public int getExpand() {
			if (expand == null)
				return tableConstraints.getExpand();
		...
		public void setExpand(Integer expand) {
			if (expand == null && this == tableConstraints)
				return;
			this.expand = expand;
			invalidate();
		}
		...
		public int[] getPadding() {
			if (padding == null)
				return tableConstraints.getPadding();
			return new int[] { padding[0].intValue(), padding[1].intValue(), padding[2].intValue(),
					padding[3].intValue() };
		}

		public void setPadding(Integer[] padding) {
			if (padding == null && this == tableConstraints)
				return;
		...
		public int[] getSpacing() {
			if (spacing == null)
				return tableConstraints.getSpacing();
			return new int[] { spacing[0].intValue(), spacing[1].intValue(), spacing[2].intValue(),
					spacing[3].intValue() };
		}

		public void setSpacing(Integer[] spacing) {
			if (spacing == null && this == tableConstraints)
				return;
		...
		public boolean getBorder() {
			if (border == null)
				return tableConstraints.getBorder();
			return border.booleanValue();
		}

		public void setBorder(Boolean border) {
			if (border == null && this == tableConstraints)
				return;
			this.border = border;
			invalidate();
		}
	...
	/**
	 * Stores constraints for the table and default constraints for the table cells.
	 * These defaults are used for all cells that don't specify a value. This means
	 * TableConstraints fields canont be set to null.
	 */
	public class TableConstraints extends Constraints {
		...
		public void configure(Map map) {
			...
			if (map.get("columns") != null)
				setColumns(Integer.parseInt((String) map.get("columns")));
			if (map.get("uniform") != null)
				setFixed((String) map.get("colspan"));

			// Legacy constraints.
			if (map.get("columns") == null && map.get("cols") != null)
				setColumns(Integer.parseInt((String) map.get("cols")));
		}
	...
	public class CellConstraints extends Constraints {
		...
		public void configure(Map map) {
			...
			if (map.get("colspan") != null)
				setColspan(Integer.parseInt((String) map.get("colspan")));
			if (map.get("rowspan") != null)
				setRowspan(Integer.parseInt((String) map.get("rowspan")));
		}
